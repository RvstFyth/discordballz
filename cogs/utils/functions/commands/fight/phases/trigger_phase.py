...
async def Passive_trigger(client, ctx, player, character_team, enemy_team, team_num):
    '''
    `coroutine`

    Trigger the passive effects.
...
async def Character_trigger(client, ctx, player, character, character_team, enemy_team):
    '''
    `coroutine`

    Trigger the effect of this character.
    ...
    `character_team`/`enemy_team` : must be list of `Character` instances.
    ...
    if(character.current_hp > 0):  # Only check if the character is still alive
        ...
        if(char_effect):
            final_display += '{}**{}**{}{} :{}```\n```'.format(character.icon, character.name, character.rarity_icon, character.type_icon, character_trigger)
